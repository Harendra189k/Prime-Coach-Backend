...
router.post("/adminlist", async (req, res) => {
    ...
    const newAdmin = new Admin({
      ...
      password:req.body.password && CryptoJS.AES.encrypt(
        req.body.password,
...
const generateTempPassword = () => {
  ...
  router.get("/getcount", async(req, res) => {
    try{
      ...
      res.status(200).json({coach:coach, athlete:athlete, addTeam: addTeam, addPlayer: addPlayer, status:200})
    } 
    catch(err){
      res.status(500).json(err)
    }
...
router.put("/change-admin-password", async (req, res) => {
  try {
    ...
    const bytes = CryptoJS.AES.decrypt(storedEncryptedPassword, process.env.PASS_SEC);
    const decryptedPassword = bytes.toString(CryptoJS.enc.Utf8);
    // console.log(decryptedPassword)

    if (decryptedPassword === req.body.oldPassword) {
      adminData.password = req.body.newPassword && CryptoJS.AES.encrypt(
        req.body.newPassword,
        process.env.PASS_SEC
      ).toString() ;
      await adminData.save();

      return res.status(200).json({ message: "Admin Password Changed Successfully", status:200 });
    } else {
